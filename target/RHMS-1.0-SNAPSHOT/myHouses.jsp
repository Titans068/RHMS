...
<div class="container">
    <h2 class="font-weight-light">My Listings</h2>
    ...
    <sql:query var="rent" dataSource="${base}"
               sql="SELECT max(rent) FROM houses WHERE username = '${username}';"/>
    <sql:query var="locations" dataSource="${base}"
               sql="SELECT DISTINCT location FROM houses WHERE username = '${username}';"/>
    ...
    <c:choose>
        <c:when test="${param.submit != null}">
            <%
                ...
                Restrict.setSubRestrict(Restrict.ME);
            %>
        </c:when>
        <c:otherwise>
            <%
                Restrict.setRestrict(Restrict.ME);
            %>
        </c:otherwise>
    </c:choose>
...
<footer class="bg-dark py-4 mt-auto">
    <div class="container px-5">
        <div class="row align-items-center justify-content-between flex-column flex-sm-row">
            ...
            <div class="col-auto">
                <div class="small m-0 text-white">Copyright © 2019 - 2022. ALL RIGHTS RESERVED. | MARK MWIRIGI.
                </div>
            </div>
        </div>
    </div>
